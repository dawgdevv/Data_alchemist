...
import { MessageSquare, Send, ChevronRight, Sparkles, Check, X } from "lucide-react"
...
export default function AIAssistant() {
  ...
  const getAIResponse = (input: string): string => {
    ...
    if (lowerInput.includes("co-run") || lowerInput.includes("t1") || lowerInput.includes("t2")) {
  ...
  const getSuggestion = (input: string): { action: string; preview: string } | undefined => {
    const lowerInput = input.toLowerCase()
  ...
  return (
    <div
      className={`bg-[#181825] border-l border-[#313244] transition-all duration-300 ${isExpanded ? "w-96" : "w-12"}`}
    >
      {/* Toggle Button */}
      <div className="p-3 border-b border-[#313244]">
        <Button
          ...
          className="w-full justify-center text-[#cdd6f4] hover:bg-[#313244]"
        >
          {isExpanded ? (
            <>
      ...
      {isExpanded && (
        <div className="flex flex-col h-[calc(100vh-121px)]">
          {/* Header */}
          <div className="p-4 border-b border-[#313244]">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-xs text-[#6c7086]">Natural language data processing</p>
              </div>
            </div>
          </div>
              ...
              <div key={message.id} className={`flex ${message.type === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] rounded-lg p-3 ${
                    message.type === "user" ? "bg-[#cba6f7] text-[#1e1e2e]" : "bg-[#313244] text-[#cdd6f4]"
                  }`}
                >
                  ...
                  {message.suggestion && (
                    <div className="mt-3 p-2 bg-[#45475a] rounded border border-[#585b70]">
                      <div className="flex items-center justify-between mb-2">
                        <Badge variant="outline" className="bg-[#a6e3a1]/20 text-[#a6e3a1] border-[#a6e3a1]/30 text-xs">
                          Suggestion
                        </Badge>
                      </div>
                      <p className="text-xs text-[#cdd6f4] font-medium mb-1">{message.suggestion.action}</p>
                      <p className="text-xs text-[#6c7086] mb-3">{message.suggestion.preview}</p>
              ...
              <div className="flex justify-start">
                <div className="bg-[#313244] text-[#cdd6f4] rounded-lg p-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-[#6c7086]">Thinking...</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="p-4 border-t border-[#313244]">
            <div className="flex space-x-2">
              <Input
                ...
                className="flex-1 bg-[#1e1e2e] border-[#45475a] text-[#cdd6f4] text-sm"
                onKeyDown={(e) => {
